...
## ASW: right! A preferrable way to assess overdispersion would be with a dispersion test of some kind, as in class, but I also think there's nothing wrong with cutting straight to the negative binomial in ecology :) 

## 1b)
plot_predictions(mod, condition = "Treatment")
predictions(mod, newdata = data.frame(Treatment = c("parasitized", "unparasitized")))
...
## ASW: How would you rephrase this to interpret the interaction itself? Nice work! 28/30
...
## ASW:  Right! The key thing here is that slope and distance from roads are biasing the effect of thinning in the first model, making it appear more effective than it is because of the fact that thinning treatments are more likely to occur in locations where fire severity is already lower (closer to roads, on shallower slopes). 
